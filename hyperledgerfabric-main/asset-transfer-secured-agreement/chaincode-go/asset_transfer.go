...
func (s *SmartContract) CreateAsset(ctx contractapi.TransactionContextInterface, assetID, publicDescription string) error {
	...
	// Get client private collection name
	...
	assetAsBytesCheck, err := ctx.GetStub().GetPrivateData(collection, asset.ID)
	if err != nil {
		return fmt.Errorf("failed to get asset: %v", err)
	} else if assetAsBytesCheck != nil {
		fmt.Println("Asset already exists: " + asset.ID)
		return fmt.Errorf("this asset already exists: " + asset.ID)
	}
