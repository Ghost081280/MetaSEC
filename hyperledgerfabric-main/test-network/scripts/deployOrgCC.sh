...
println "- CC_ORGNIZATION: ${C_GREEN}${CC_ORGNIZATION}${C_RESET}"
println "- CC_ORGNIZATION_PORT: ${C_GREEN}${CC_ORGNIZATION_PORT}${C_RESET}"
...
println "FABRIC_CFG_PATH : ${FABRIC_CFG_PATH}";
...
approveForMyOrg() {
  ...
  setGlobals $ORG $PORT
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.o3.fit --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.o3.fit --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} "${PEER_CONN_PARMS[@]}" --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.o3.fit --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}" --isInit -c ${fcn_call} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
infoln "Installing chaincode on peer0.${CC_ORGNIZATION}..."
installChaincode ${CC_ORGNIZATION} ${CC_ORGNIZATION_PORT}

## query whether the chaincode is installed
queryInstalled ${CC_ORGNIZATION} ${CC_ORGNIZATION_PORT}

## check whether the chaincode definition is ready to be committed
## expect org1 to have approved and org2 not to
#checkCommitReadiness 'org1' "\"Org1MSP\": true" "\"Org2MSP\": false"
#checkCommitReadiness 'org2' "\"Org1MSP\": true" "\"Org2MSP\": false"

## now approve also for org2
approveForMyOrg ${CC_ORGNIZATION} ${CC_ORGNIZATION_PORT}
...
if [ "$CC_INIT_FCN" = "NA" ]; then
  ...
  chaincodeInvokeInit CC_ORGNIZATION_PORT
fi
