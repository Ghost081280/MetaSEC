...
class TokenERC721Contract extends Contract {
    ...
    */
    async MintWithTokenURI(ctx, amount, tokenURI) {
        ...
        // let exists = true;
        // while(exists){
        //     tokenId = this._rnd256();
        //     // tokenId = Math.floor(Math.random() * 10000000000000000000);
        //     if(!this._nftExists(ctx, tokenId)) {
        //         exists = false;
        //     }
        // }

        const tokenIdFNC = async () => {
            let tokenRand;
            while (true) {
                tokenRand = crypto.randomBytes(32).toString("hex");
                const exists = await this._nftExists(ctx, tokenRand);
                if (exists) {
        ...
        const tokenId = await tokenIdFNC();
        ...
        // if(exists) {
        //     const NFFT = this._readNFT(ctx, tokenId);
        //     throw new Error('mint amount must be a positive integer : '+ JSON.stringify(NFFT));
        // }
    ...
    /**
     * ClientMintedNFTs returns the balance of the requesting client's account.
     *
     * @param {Context} ctx the transaction context
     * @returns {Number} Returns the account balance
     */
    async ClientMintedNFTs(ctx) {
        // Get ID of submitting client identity
        const clientAccountID = ctx.clientIdentity.getID();
        return this.QueryAssetsByOwner(ctx, clientAccountID);
    }
